            ...
            notificacoesExibidas.map(notif => (
              <div key={notif.id} className={`notificacao-card ${notif.lida ? '' : 'unread'}`}>
                <div className="card-left">
                  <div className="type-icon-container"><span className="type-icon">{renderIcon(notif.tipo)}</span></div>
                ...
                <div className="card-right">
                  <button className={`action-small-outline ${notif.lida ? 'is-read' : 'is-unread'}`} onClick={() => handleToggleReadStatus(notif.id)} title={notif.lida ? "Marcar como não lida" : "Marcar como lida"}>
                    {notif.lida ? '👁️‍🗨️' : '✓'} 
                  </button>
                  <button className="action-small-outline" onClick={() => handleDelete(notif.id)} title="Apagar">🗑️</button>
                </div>
              </div>
            ))
          ) : (
